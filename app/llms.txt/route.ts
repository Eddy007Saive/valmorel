...
export function GET() {
  const cities = CITIES.map((c) => `- [Conciergerie à ${c.name}](https://valmorel.cledici.fr/conciergerie/${c.slug}) — ${c.intro}`).join("\n");
  const articles = ARTICLES.map((a) => `- [${a.title}](https://valmorel.cledici.fr/blog/${a.slug}) — ${a.excerpt}`).join("\n");
...
## Pages principales
- [Accueil](https://valmorel.cledici.fr/)
- [Sécurité & réglementation à Valmorel](https://valmorel.cledici.fr/securite-reglementation) — déclaration en mairie, classement, taxe de séjour par classement, assurance.
- [Nos interventions](https://valmorel.cledici.fr/nos-interventions)

    ...
    <>
      ...
      <article className="sec" style={{ paddingTop: 48 }}>
        <div className="wrap" style={{ maxWidth: 820 }}>
          ...
          <p className="eyebrow" style={{ marginTop: 22 }}>{fmtDate(a.date)}</p>
          <h1 style={{ fontSize: "clamp(28px,3.8vw,44px)", fontWeight: 800, lineHeight: 1.14, marginTop: 12 }}>{a.title}</h1>
          <p style={{ marginTop: 16, fontSize: 18, lineHeight: 1.6, color: "var(--muted)" }}>{a.excerpt}</p>
          <img src={a.cover} alt={a.title} style={{ width: "100%", height: "clamp(260px,42vh,440px)", objectFit: "cover", borderRadius: 16, margin: "28px 0 8px" }} />

          <div className="prose" dangerouslySetInnerHTML={{ __html: a.contentHtml }} />

          <div style={{ marginTop: 40, background: "var(--green)", color: "#fff", borderRadius: 18, padding: "34px 32px", textAlign: "center" }}>
        ...
        <section className="sec" style={{ background: "var(--snow)", paddingTop: 64, paddingBottom: 80 }}>
          <div className="wrap">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: 24, marginTop: 36 }}>
              {others.map((p) => (
                <Link key={p.slug} href={`/blog/${p.slug}`} style={{ borderRadius: 16, overflow: "hidden", border: "1px solid var(--line)", background: "#fff", display: "block" }}>
